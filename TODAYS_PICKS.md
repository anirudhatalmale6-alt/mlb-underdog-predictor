# MLB Picks - Friday, August 28, 2026

*Last updated: 2026-08-28 11:30 PM ET*

## Moneyline Underdogs

### RECOMMENDED PLAYS (3)

#### ??? (0)

| Stat | Value |
|------|-------|
| Matchup | LAD @ DET |
| Starting Pitchers | Tarik Skubal vs Drew Anderson |
| Model Win Probability | 0 |
| Edge Over Market | 7.3% |
| Confidence | HIGH |
| Notes | Elite pitching matchup |

#### ??? (0)

| Stat | Value |
|------|-------|
| Matchup | TEX @ MIL |
| Starting Pitchers | Cody Bradford vs Logan Henderson |
| Model Win Probability | 0 |
| Edge Over Market | 4.8% |
| Confidence | MEDIUM |
| Notes | Elite pitching matchup |

#### ??? (0)

| Stat | Value |
|------|-------|
| Matchup | SD @ TB |
| Starting Pitchers | Casey Mize vs Shane McClanahan |
| Model Win Probability | 0 |
| Edge Over Market | 8.2% |
| Confidence | LOW |
| Notes | Standard play |

---

## 1st Inning Moneyline

### RECOMMENDED PLAYS (2)

| Matchup | Pitchers | Pick | Odds | Probability | Edge | Confidence | Notes |
|---------|----------|------|------|-------------|------|------------|-------|
| LAD @ DET | Tarik Skubal vs Drew Anderson | LAD | -110 | 59.6% | 7.3% | HIGH | Elite pitching matchup |
| TEX @ MIL | Cody Bradford vs Logan Henderson | TEX | -110 | 57.1% | 4.8% | MEDIUM | Elite pitching matchup |

---

## 1st Inning Over/Under

### RECOMMENDED PLAYS (1)

| Matchup | Pitchers | Pick | Odds | Probability | Edge | Confidence | Notes |
|---------|----------|------|------|-------------|------|------------|-------|
| SD @ TB | Casey Mize vs Shane McClanahan | OVER 0.5 | -110 | 60.5% | 8.2% | LOW | Standard play |

---

---

### How to Read This

**Moneyline Underdogs:**
- **Odds**: The moneyline payout (e.g., +150 means $100 bet wins $150)
- **Win Probability**: The model's estimated chance of this underdog winning
- **Edge**: How much higher the model's probability is vs what the odds imply (positive = value bet)

**Run Line (+1.5):**
- **Pick**: The underdog team +1.5 runs (covers if they win or lose by 1)
- **Odds**: The payout odds for the run line bet
- **Cover Prob**: The model's estimated chance the underdog covers +1.5

**Totals (Over/Under):**
- **Line**: The sportsbook's projected total runs for the game
- **Pick**: OVER or UNDER — the model's prediction
- **Probability**: The model's confidence in its pick
- **Edge**: How much the model disagrees with the 50/50 line (higher = more confident)

**1st Inning Markets:**
- **ML**: Which team the model favors to win the 1st inning (ties push)
- **Total**: OVER or UNDER 0.5 runs in the 1st inning
- **Odds**: The payout odds for the bet

**Player Props:**
- **Player**: The player and their matchup
- **Pick**: OVER or UNDER the prop line (e.g., OVER 6.5 Ks)
- **Odds**: The payout odds for the prop

**Confidence Levels:** HIGH (12%+ edge), MEDIUM (10-12%), LOW (8-10%)

*Disclaimer: This is a statistical model for informational purposes. Past performance does not guarantee future results. Gamble responsibly.*